...
const files=await walk('.');for(const file of files){const text=await readFile(file,'utf8');if(text.includes('TODO_FAKE')) throw new Error(`Lint marker found in ${file}`);}console.log(`Linted ${files.length} files`);
